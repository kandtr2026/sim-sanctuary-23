...
const SITE = 'https://www.chonsomobifone.com';
...
const PolicyPage = () => {
  ...
  // Routes are declared as explicit static paths in App.tsx, so derive the
  // document key from the URL rather than a route param.
  const slug = pathname.replace(/^\/+|\/+$/g, '');
  const doc = POLICY_DOCS[slug];

  // Unknown slug under this route → real 404, not a silent redirect.
  if (!doc) return <NotFound />;

  const canonical = `${SITE}/${doc.slug}`;

  return (
    <div className="min-h-screen bg-background">
      <Helmet>
        <title>{`${doc.title} | CHONSOMOBIFONE.COM`}</title>
        <meta name="description" content={doc.description} />
        <link rel="canonical" href={canonical} />
        <meta property="og:type" content="website" />
        <meta property="og:url" content={canonical} />
        <meta property="og:title" content={`${doc.title} | CHONSOMOBIFONE.COM`} />
